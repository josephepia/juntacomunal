...
export class NotificacionesService {
 ...
 // type LogLevelStrings = keyof typeof ErrorCode;
  ...
  modalInformativo(informacion:{titulo: string, descripcion?: any, errorCode?: string}){
 
    
    let codeErrorKeys = Object.keys(this.ErrorCode)

    if(informacion.errorCode){
      let code = codeErrorKeys.filter((code) => (informacion.errorCode ||'').toString().includes(code))
      // let indexError = codeErrorKeys.indexOf(informacion.descripcion)
      // let nuevaDescripcion = Object.values(ErrorCode)[indexError]
      informacion.descripcion = this.ErrorCode[code[0]];
    }
  ...
  ErrorCode: any = {
    ...
    "permission_denied": "El cliente no tiene permiso para acceder a los datos deseados."
  }
}
